...
cmap_myc_ranks_landmark$pvalue.pathcancer <- 666
...
cmap_myc_ranks_landmark$padj.pathcancer <- 666
...
cmap_myc_ranks_landmark$rankNES.pathcancer <- 666
...
cmap_myc_ranks_landmark$NES.pathcancer <- 666
for (i in 1:NCOL(cmap_PA)) {
  ...
  cmap_myc_ranks_landmark$pvalue.pathcancer[which(cmap_myc_ranks_landmark$sig_id==colnames(cmap_PA)[i])]<-  paths %>% filter(pathway=="hsa05200 Pathways in cancer") %>% select(pval)
  ...
  cmap_myc_ranks_landmark$padj.pathcancer[which(cmap_myc_ranks_landmark$sig_id==colnames(cmap_PA)[i])]<-  paths %>% filter(pathway=="hsa05200 Pathways in cancer") %>% select(padj)
  ...
  cmap_myc_ranks_landmark$rankNES.pathcancer[which(cmap_myc_ranks_landmark$sig_id==colnames(cmap_PA)[i])]<-  paths %>% filter(pathway=="hsa05200 Pathways in cancer") %>% select(NES.rank)
  ...
  cmap_myc_ranks_landmark$NES.pathcancer[which(cmap_myc_ranks_landmark$sig_id==colnames(cmap_PA)[i])]<-  paths %>% filter(pathway=="hsa05200 Pathways in cancer") %>% select(NES)
  
  
  paths <- NULL
  saveRDS(cmap_myc_ranks_landmark,"pathway_results_myc.rds")
